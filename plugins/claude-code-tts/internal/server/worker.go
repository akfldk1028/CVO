...
func (wp *WorkerPool) processJob(job *Job) {
	...
	audioData, err := wp.ttsClient.Synthesize(job.Text, job.Voice, job.Instructions)
	if err != nil {
		job.mu.Lock()
		job.Status = "failed"
		job.Error = err.Error()
...
func (wp *WorkerPool) Submit(text string, voice tts.Voice, instructions ...string) (*Job, error) {
	job := &Job{
	...
	if len(instructions) > 0 {
		job.Instructions = instructions[0]
	}
