...
func (s *Server) registerTools() {
	...
	speakTool := mcp.NewTool("speak",
		...
		mcp.WithString("voice",
			mcp.Description("Voice to use: alloy, ash, ballad, coral, echo, fable, onyx, nova, sage, shimmer (default: coral)"),
		),
		mcp.WithString("instructions",
			mcp.Description("Instructions for how the text should be spoken, e.g. tone, emotion, style. Only works with gpt-4o-mini-tts model."),
		),
	)
...
func (s *Server) handleSpeak(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Extract voice parameter (default to coral)
	voice := "coral"
	...
	if !tts.IsValidVoice(voice) {
		...
		return mcp.NewToolResultError(fmt.Sprintf("invalid voice '%s'. Valid voices: alloy, echo, fable, onyx, nova, shimmer, coral", voice)), nil
	}

	// Extract instructions parameter
	instructions := ""
	if inst, ok := request.Params.Arguments["instructions"].(string); ok {
		instructions = inst
	}
	...
	job, err := s.workerPool.Submit(text, tts.Voice(voice), instructions)
	if err != nil {
